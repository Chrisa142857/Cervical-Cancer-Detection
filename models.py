...
def create_modules(module_defs, fp_max=0.5, use_final_loss=False, use_mish=False, use_fp_score=False, old_version=False, use_focal_loss=False):
    """
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            if module_def["activation"] == "leaky" and not use_mish:
            ...
            yolo_layer = YOLOLayer(anchors, num_classes, fp_max=fp_max, use_final_loss=use_final_loss, use_fp_score=use_fp_score, old_version=old_version, use_focal_loss=use_focal_loss)
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, fp_max=0.5, use_final_loss=False, use_fp_score=False, old_version=False, use_focal_loss=False):
    ...
    def forward(self, x, targets=None, img_dim=None, fp_flag=False):
        ...
        if not targets is None:
            ...
            if not fp_isZero:
                loss_final_conf_obj = self.bce_loss(pred_final_conf[fp_mask], tconf[fp_mask])
                loss_fp_conf_obj = self.bce_loss(pred_fp_conf[fp_mask], tfpconf[fp_mask])
                ...
                loss_final_conf_obj = nn.BCELoss()(pred_final_conf[fp_mask], tconf[fp_mask])
                loss_fp_conf_obj = nn.BCELoss()(pred_fp_conf[fp_mask], tfpconf[fp_mask])
                self.fp_trained_flag = False
                loss_conf = loss_gt
            ...
            output = torch.cat(
                (
                    ...
                    pred_final_conf.view(num_samples, -1, 1),
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, fp_max=0.5, use_final_loss=True, use_mish=False, use_fp_score=True, old_version=False, use_focal_loss=True, lite_mode=False, debug_mode=False):
        super(Darknet, self).__init__()
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_modules(self.module_defs, fp_max=fp_max,
    ...
    def forward(self, x, targets=None, fp_flag=False):
