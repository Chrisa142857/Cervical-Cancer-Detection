...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_id", type=str, default="0", help="CUDA setting")
    parser.add_argument("--learning_rate", type=float, default=0.0001)
    parser.add_argument("--lr_max_step", type=int, default=400000, help="Cosine lr_schedule setting")
    parser.add_argument("--folder", type=str, default="images")
    parser.add_argument("--eval_folder", type=str, default="images")
    parser.add_argument("--fp_record", type=bool, default=True, help="Auto FP learning")
    parser.add_argument("--fp_interval", type=int, default=2, help="FP changing interval")
    parser.add_argument("--neg_sample_interval", type=int, default=2, help="Negative img sample interval")
    parser.add_argument("--fp_sample_interval", type=int, default=10, help="FP label sample interval")
    parser.add_argument("--start_step", type=int, default=0)
    parser.add_argument("--start_epoch", type=int, default=0)
    parser.add_argument("--use_fp_layer", type=bool, default=False, help="True if fp_record")
    parser.add_argument("--use_final_loss", type=bool, default=True, help="True is best")
    parser.add_argument("--use_mish", type=bool, default=False, help="True is best but with double GPU RAM")
    parser.add_argument("--use_fp_score", type=bool, default=True, help="True is best")
    parser.add_argument("--load_neck_only", type=bool, default=False, help="Load neck only")
    parser.add_argument("--fp_restrain", type=bool, default=True, help="True is best")
    parser.add_argument("--use_focal_loss", type=bool, default=True, help="True is best")
    parser.add_argument("--skip_first_train", type=bool, default=False, help="Skip first train")
    parser.add_argument("--group_conv", type=bool, default=False, help="ignore")
    parser.add_argument("--lite_mode", type=bool, default=False, help="ignore")
    ...
    # Initiate model
    model = Darknet(opt.model_def, use_final_loss=opt.use_final_loss, use_mish=opt.use_mish, use_fp_score=opt.use_fp_score, use_focal_loss=opt.use_focal_loss, lite_mode=opt.lite_mode).to(device)
    ...
    if opt.pretrained_weights:
        if not opt.load_neck_only:
            ...
            model.load_darknet_weights(opt.pretrained_weights, use_torch=True, cutoff=74)
    ...
    # model = nn.DataParallel(model, device_ids=[0,1]) # multi-GPU (ignore)
    metrics = [
        "loss",
        "loss_conf",
        "loss_conf_obj",
    ...
    for epoch in range(opt.start_epoch, opt.epochs):
        ...
        if not (epoch == opt.start_epoch and opt.skip_first_train):
            ...
            for batch_i, (img_path, imgs, targets) in enumerate(dataloader):
                ...
                if (batches_done + NEG_SAMPLE_INTERVAL) % opt.checkpoint_interval == 0:
                    torch.save(model.state_dict(), f"%s/yolov3_ckpt_step_%d.pth" % (opt.save_path, batches_done))
                    print("\n---- Evaluating Model ----")
                    # Evaluate the model on the validation set
                    precision, recall, AP, f1, ap_class, val_fp_nums = evaluate(
                        model,
                        path=valid_path,
                        iou_thres=0.5,
                        conf_thres=0.5,
        ...
        if epoch % opt.evaluation_interval == 0:
            ...
            torch.save(model.state_dict(), f"%s/yolov3_ckpt_epoch_%d.pth" % (opt.save_path, epoch))
